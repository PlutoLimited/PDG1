...
class TICTask : public smooth::core::Task {
 ...
 private:
  ...
  void printOutputData() {
    ...
    if (ticOutput.m_deviceState ==
        runnable::tic::EDeviceState::DEVICE_NOT_PRESENT) {
      Log::error(G_TASK_TAG, "IQS263B is NOT present");
    } else {
      Log::info(G_TASK_TAG, "IQS263B is present");
    }
    ...
    Log::info(G_TASK_TAG, stack_format, "Device State",
