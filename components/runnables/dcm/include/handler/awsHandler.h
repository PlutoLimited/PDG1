...
static void windowActuate_Callback(const char *pJsonString,
  ...
  if (pContext != NULL) {
    ESP_LOGI(TAG, "Delta - Window state changed to %d",
             *(bool *)(pContext->pData));
  }
}

static void aws_iot_test() {
  ...
  if (SUCCESS != rc) {
    ESP_LOGE(TAG,
             "aws_iot_shadow_init returned error %d, "
             "aborting...",
             rc);
    abort();
  }
  ...
  if (SUCCESS != rc) {
    ESP_LOGE(TAG,
             "aws_iot_shadow_connect returned error %d, "
             "aborting...",
             rc);
    abort();
  }
  ...
  if (SUCCESS != rc) {
    ESP_LOGE(TAG,
             "Unable to set Auto Reconnect to true - %d, "
             "aborting...",
             rc);
    abort();
  }
  ...
  while (NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc ||
    ...
    if (NETWORK_ATTEMPTING_RECONNECT == rc || shadowUpdateInProgress) {
      ...
      // If the client is attempting to reconnect, or
      // already waiting on a shadow update, we will skip
    ...
    if (SUCCESS == rc) {
      rc = aws_iot_shadow_add_reported(JsonDocumentBuffer,
                                       sizeOfJsonDocumentBuffer, 2,
                                       &temperatureHandler, &windowActuator);
      if (SUCCESS == rc) {
        rc = aws_iot_finalize_json_document(JsonDocumentBuffer,
                                            sizeOfJsonDocumentBuffer);
        if (SUCCESS == rc) {
          ESP_LOGI(TAG, "Update Shadow: %s", JsonDocumentBuffer);
          rc = aws_iot_shadow_update(&mqttClient, CONFIG_AWS_IOT_THING_NAME,
                                     JsonDocumentBuffer,
